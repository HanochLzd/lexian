            ...
            if (confirm('确定吗？' + $scope.oData.username)) {
                httpService.getRequest('manager/signOut.do', {})
                    .then(function (data) {
                        ...
                        window.location.pathname = '/lexian';
                    }, function (error) {
...
myApp.controller('goodsCategoryController', ['$scope', '$state', '$timeout', 'httpService',
    function ($scope, $state, $timeout, httpService) {
        $scope.selctedFilter = 'categoryName';
        $scope.filterStr = '';
        $scope.filterTypes = [{chName: '分类名称', filterName: 'categoryName'}];

        // 一级分类
        $scope.pageNo1 = 1;
        $scope.pageSize1 = 10;
        $scope.getTotalCategory = function () {
            $timeout(function () {
                httpService.getRequest('categoryView/getCategoryView.do', {
                    params: {
                    ...
                    .then(function (data) {
                        if (1 === data.code) {
                            $scope.totalCategory = data.data.data;
